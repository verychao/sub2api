      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="canSyncUpstreamBalance" @click="$emit('sync-balance', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-cyan-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="sync" size="sm" />
              {{ t('admin.accounts.syncUpstreamBalance') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'schedule', 'sync-balance', 'reauth', 'refresh-token', 'recover-state', 'reset-quota', 'set-privacy'])
const { t } = useI18n()
...
const canSyncUpstreamBalance = computed(() => props.account?.platform === 'openai' && props.account?.type === 'apikey')
